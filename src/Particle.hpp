...
public:
  ...
  CUDA_FUNC_DEC ~Particle() =
      default; // should not be virtual (in case of using CUDA)
  [[nodiscard]] CUDA_FUNC_DEC P_CONSTEXPR float
  dist2(const Particle &p) const noexcept;
  [[nodiscard]] CUDA_FUNC_DEC P_CONSTEXPR float
  dist(const Particle &p) const noexcept;
