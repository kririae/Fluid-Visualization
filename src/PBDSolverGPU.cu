...
#define OBJECT_PARAM float border, float radius, float epsilon,

PBDSolverGPU::PBDSolverGPU(float border, float radius, float epsilon)
...
void
PBDSolverGPU::updateGUI(RTGUI *gui_ptr) noexcept
{
  for (int i = 0; i < n_substeps; ++i)
    this->subStep();
  gui_ptr->setParticles(data);
}

void
PBDSolverGPU::subStep()
{
  ...
  apply_force<<<num_blocks, threads_per_block> > >(RAW_PTR(dev_data), data_size,
                                                   ext_f, delta_t);
  ...
  // Block of building
  {
    ...
    int *dev_hash_map;
    size_t pitch_hash;
    CUDA_CALL(cudaMallocPitch(&dev_hash_map, &pitch_hash,
                              MAX_NEIGHBOR_SIZE * sizeof(int), // risk of exceed
                              map_size));
    CUDA_CALL(cudaMallocPitch(&dev_neighbor_map, &pitch_neighbor,
                              MAX_NEIGHBOR_SIZE * sizeof(int), data_size));
...
void PBDSolverGPU::addParticle(const SPHParticle &p)
{
  data.push_back(p);
}


__device__ void PBDSolverGPU::constraint_to_border(SPHParticle &p)
{
...
// On both platform
CUDA_FUNC_DEC
float PBDSolverGPU::poly6(float r, float h) noexcept
{
...
CUDA_FUNC_DEC
vec3 PBDSolverGPU::grad_spiky(vec3 v, float h) noexcept
{
...
CUDA_FUNC_DEC
float PBDSolverGPU::computeSCorr(const SPHParticle &p_i,
                                ...
                                float delta_t)
{
  ...
  for (; i < data_size; i += blockDim.x * gridDim.x) {
    auto &p = dev_data[i];
    PBDSolverGPU::constraint_to_border(p);
...
__global__ void apply_force(
  ...
  for (; i < data_size; i += blockDim.x * gridDim.x) {
    ...
    PBDSolverGPU::constraint_to_border(dev_data[i]);
  }
}
...
__global__ void fill_lambda(
  ...
  for (; i < data_size; i += blockDim.x * gridDim.x) {
    ...
    for (int index = 0; index < dev_n_neighbor_map[i]; ++index) {
      ...
      if (i == j) {
        for (int k = 0; k < dev_n_neighbor_map[i]; ++k) {
          ...
          grad_c += PBDSolverGPU::grad_spiky(
              dev_data[i].pos - dev_data[neighbor_index].pos, radius);
        }
      } else {
        grad_c = -PBDSolverGPU::grad_spiky(dev_data[i].pos - dev_data[j].pos,
                                        radius);
      }
    ...
    lambda[i] = -c_i[i] / (_denom + denominatorEpsilon);
  }
}
...
__global__ void apply_motion(
  ...
  for (; i < data_size; i += blockDim.x * gridDim.x) {
    ...
    for (int index = 0; index < dev_n_neighbor_map[i]; ++index) {
      ...
      delta_p_i +=
          (lambda[i] + lambda[j] +
           PBDSolverGPU::compute_s_corr(dev_data[i], dev_data[j], radius)) *
          PBDSolverGPU::grad_spiky(dev_data[i].pos - dev_data[j].pos, radius);
    }
...
__global__ void build_hash_map(
  ...
  unsigned int i = threadIdx.x + blockIdx.x * blockDim.x;
  for (; i < data_size; i += blockDim.x * gridDim.x) {
    int hash_map_index = PBDSolverGPU::hash(dev_data[i].pos, n_grids);
    int *dev_hash_map_item = (int *)((char *)dev_hash_map +
                                     pitch_hash * hash_map_index);

    while (atomicCAS(&hash_map_mutex[hash_map_index], 0, 1) != 0)
      ;
    if (dev_n_hash_map[hash_map_index] < PBDSolverGPU::MAX_NEIGHBOR_SIZE) {
...
__global__ void build_neighbor_map(
  ...
  for (; i < data_size; i += blockDim.x * gridDim.x) {
    ...
    const auto grid_index = PBDSolverGPU::get_grid_index(center.pos);
    ...
    for (int u = grid_index.x - 1; u <= grid_index.x + 1; ++u) {
      for (int v = grid_index.y - 1; v <= grid_index.y + 1; ++v) {
        for (int w = grid_index.z - 1; w <= grid_index.z + 1; ++w) {
          ...
          const int hash_map_index = PBDSolverGPU::hash_from_grid(
              u, v, w, n_grids);
          int *dev_hash_map_item = (int *)((char *)dev_hash_map +
                                           pitch_hash * hash_map_index);
          for (int k = 0; k < dev_n_hash_map[hash_map_index]; ++k) {
            ...
            if (center.dist2(dev_data[j]) <= radius2 &&
                dev_n_neighbor_map[i] < PBDSolverGPU::MAX_NEIGHBOR_SIZE) {
...
CUDA_FUNC_DEC
inline int PBDSolverGPU::hash(float x, float y, float z, int n_grids)
{
...
CUDA_FUNC_DEC
inline int PBDSolverGPU::hash(const vec3 &p, int n_grids)
{
  return hash(p.x, p.y, p.z, n_grids);
}

CUDA_FUNC_DEC
inline ivec3 PBDSolverGPU::get_grid_index(const vec3 &p)
{
...
CUDA_FUNC_DEC
inline int PBDSolverGPU::hash_from_grid(int u, int v, int w, int n_grids)
{
  return u + v * n_grids + w * n_grids * n_grids;
}

CUDA_FUNC_DEC
inline int PBDSolverGPU::hash_from_grid(const ivec3 &p, int n_grids)
{
