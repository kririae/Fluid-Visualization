...
#ifndef FLUIDVISUALIZATION_SRC_MESHER_VDBMESHER_HPP
#define FLUIDVISUALIZATION_SRC_MESHER_VDBMESHER_HPP
...
class ParticleList {
public:
  explicit ParticleList(const std::vector<vec3> &_data);
...
void particleToMesh(const std::vector<vec3> &data,
                    std::vector<openvdb::Vec3s> &points,
                    std::vector<openvdb::Vec3I> &triangles,
                    std::vector<openvdb::Vec4I> &quads);

#endif //FLUIDVISUALIZATION_SRC_MESHER_VDBMESHER_HPP
