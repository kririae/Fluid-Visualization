...
CUDA_FUNC_DEC P_CONSTEXPR float
Particle::dist2(const Particle &p) const noexcept
{
...
CUDA_FUNC_DEC P_CONSTEXPR float
Particle::dist(const Particle &p) const noexcept
{
  return glm::sqrt(Particle::dist2(p));
}

CUDA_FUNC_DEC P_CONSTEXPR
SPHParticle::SPHParticle(vec3 _pos)
    : Particle::Particle(_pos)
{
}

CUDA_FUNC_DEC P_CONSTEXPR
SPHParticle::SPHParticle(float x, float y, float z)
    : Particle(x, y, z)
{
}
...
CUDA_FUNC_DEC P_CONSTEXPR SPHParticle &
